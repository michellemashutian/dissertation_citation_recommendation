...
from text_rnn import TextRNN, Config
import math
...
def run(args):
    ...
    print("Loading data...")
    x1_text, x2_text, y_train = data_helpers.load_data_and_labels(args.path + 'rnn-train-'+args.train+'.tsv')
    t1_text, t2_text, y_test = data_helpers.load_data_and_labels(args.path + 'rnn-test-'+args.test+'.tsv')
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        for i in range(args.num_epochs):
            for j in range(STEPS):
                ...
                sess.run(rnn.train_op,
                         feed_dict={rnn.input_x1: x1_train1, rnn.input_x2: x2_train1, rnn.input_y: y_train1})

                if j%200 == 0:
                    model_loss = sess.run(rnn.loss, feed_dict={rnn.input_x1: x1_test, rnn.input_x2: x2_test,
                                                               rnn.input_y: y_test})
                    if min_cross > model_loss:
                        min_cross = model_loss
                        print("/epoch_%s-batch_%s-total_cross_entropy_%s" % (i, j, model_loss))
                        output1 = open((args.result_path + 'result-rnn-glove-'+args.train+'-'+args.test), 'w')
                        output2 = open((args.result_path + 'vector-rnn-glove-'+args.train+'-'+args.test), 'w')
                        y_p, final_pre = sess.run((rnn.result, rnn.last), feed_dict={rnn.input_x1: x1_test,
                                                                                     rnn.input_x2: x2_test,
                                                                                     rnn.input_y: y_test})
                        for pred in y_p:
