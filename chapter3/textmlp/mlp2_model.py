...
class CitationRecNet(object):
    ...
    def MLP(self):
        ...
        with tf.variable_scope("layer1"):
            self.W11 = tf.get_variable("w11", initializer=tf.random_normal([self.x_dim1*2, self.layer1_dim], stddev=0.1),
                                      dtype=tf.float32)
            self.W12 = tf.get_variable("w12", initializer=tf.random_normal([self.x_dim2*2, self.layer1_dim], stddev=0.1),
                                      dtype=tf.float32)
            self.W13 = tf.get_variable("w13", initializer=tf.random_normal([self.x_dim3*2, self.layer1_dim], stddev=0.1),
                                      dtype=tf.float32)
            ...
            self.b13 = tf.get_variable("b13", initializer=tf.zeros([self.layer1_dim]), dtype=tf.float32)

        with tf.variable_scope("layer2"):
            self.W2 = tf.get_variable("w21",
                                      initializer=tf.random_normal([self.layer1_dim*3, self.layer2_dim], stddev=0.1),
                                      dtype=tf.float32)
            self.b2 = tf.get_variable("b21", initializer=tf.zeros([self.layer2_dim]), dtype=tf.float32)
        ...
        lsi = tf.concat([self.xalsi, self.xblsi], axis=1)
        lda = tf.concat([self.xalda, self.xblda], axis=1)
        d2v = tf.concat([self.xad2v, self.xbd2v], axis=1)


        # layer 1
        hidden11 = tf.nn.relu(tf.matmul(lsi, self.W11) + self.b11)
        hidden12 = tf.nn.relu(tf.matmul(lda, self.W12) + self.b12)
        hidden13 = tf.nn.relu(tf.matmul(d2v, self.W13) + self.b13)
        hidden11_drop = tf.nn.dropout(hidden11, self.dropout_keep)
        hidden12_drop = tf.nn.dropout(hidden12, self.dropout_keep)
        hidden13_drop = tf.nn.dropout(hidden13, self.dropout_keep)

        # layer 2
        self.hidden2 = tf.nn.elu(tf.matmul(tf.concat([hidden11_drop, hidden12_drop, hidden13_drop], axis=1), self.W2) + self.b2)
