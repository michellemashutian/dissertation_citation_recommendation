...
from text_cnn import TextCNN, Config
import math
...
def run(args):
    ...
    print("Loading data...")
    x1_text, x2_text, y_train = data_helpers.load_data_and_labels(args.path + 'cnn-train-'+args.train+'.tsv')
    t1_text, t2_text, y_test = data_helpers.load_data_and_labels(args.path + 'cnn-test-'+args.test+'.tsv')
    ...
    # max_document_length = max(max([len(x.split(" ")) for x in x1_text]),  max([len(x.split(" ")) for x in x2_text])) + 1
    ...
    # configg.gpu_options.per_process_gpu_memory_fraction = 0.25
        ...
        for i in range(args.num_epochs):
            ...
            for j in range(STEPS):
                ...
                sess.run(cnn.train_op,
                         feed_dict={cnn.input_x1: x1_train1, cnn.input_x2: x2_train1, cnn.input_y: y_train1})

                if j%200 == 0:
                    model_loss = sess.run(cnn.loss, feed_dict={cnn.input_x1: x1_test, cnn.input_x2: x2_test,
                                                               cnn.input_y: y_test})
                    if min_cross > model_loss:
                        min_cross = model_loss
                        print("/epoch_%s-batch_%s-total_cross_entropy_%s" % (i, j, model_loss))
                        output1 = open((args.result_path + 'result-cnn-glove-'+args.train+'-'+args.test), 'w')
                        output2 = open((args.result_path + 'vector-cnn-glove-'+args.train+'-'+args.test), 'w')
                        y_p, final_pre = sess.run((cnn.result, cnn.h_final), feed_dict={cnn.input_x1: x1_test,
                                                                                        cnn.input_x2: x2_test,
                                                                                        cnn.input_y: y_test})
                        for pred in y_p:
