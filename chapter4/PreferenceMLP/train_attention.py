...
from model_attention import Config, CitationRecNet
...
def parse_args():
    ...
    parser.add_argument('--epoch', type=int, default=6, help=' ')
            ...
            for j in range(STEPS):
                ...
                if j % 1 == 0:
                    ...
                    if min_cross > total_cross_entropy:
                        ...
                        y_p2 = sess.run(model.y_pred_softmax, feed_dict={model.xa: train_X1, model.xb: train_X2, model.dropout_keep: 1.0})
                        for pred2 in y_p2:
                            output.write(' '.join(str(v) for v in pred2)+'\n')
                        output.close()
