...
import numpy as np
import data_helper_net1 as data_helpers
from text_net1 import TextNet1
...
def parse_args():
    ...
    # parser.add_argument('--embedding_path', default='E:\\deal\\pubmed\\chapter5\\process\\data\\embedding.txt',
    #                     help='embedding path')
    ...
    parser.add_argument('--train', default='1', help='data path')
    parser.add_argument('--test', default='1', help='data path')
...
def run(args):
    ...
    print("Loading data...")
    x1_citation, x1_title, x1_content, y1_l1 = data_helpers.load_data_and_labels(
        args.path + 'train-' + args.train + '.tsv')
    x2_citation, x2_title, x2_content, y2_l1 = data_helpers.load_data_and_labels(
        args.path + 'test-' + args.test + '.tsv')
        ...
        for i in range(args.num_epochs):
            ...
            for j in range(STEPS):
                ...
                sess.run(birnnatt.train_op,
                         feed_dict={birnnatt.input_citation: x1_citation1,
                                    birnnatt.input_title: x1_title1,
                                    birnnatt.input_content: x1_content1,
                                    birnnatt.input_l1: y1_l11})

                if j % 200 == 0:
                    model_loss = sess.run(birnnatt.loss, feed_dict={birnnatt.input_citation: x2_citation,
                    ...
                    if min_cross > model_loss:
                        ...
                        output1 = open((args.result_path + 'net1-result.txt'), 'w')
                        y_p = sess.run(birnnatt.result,
