...
import numpy as np
import data_helper_net6 as data_helpers
from text_net6 import TextNet6
...
def parse_args():
    ...
    parser.add_argument('--train', default='53', help='data path')
    parser.add_argument('--test', default='53', help='data path')
    parser.add_argument('--para', default='0.2', help='data path')
...
def run(args):
    ...
    print("Loading data...")
    x1_citation, x1_tp, x1_title, x1_content, y1_l1, y1_l2 = data_helpers.load_data_and_labels(args.path + 'train-'+args.train+'.tsv')
    x2_citation, x2_tp, x2_title, x2_content, y2_l1, y2_l2 = data_helpers.load_data_and_labels(args.path + 'test-'+args.test+'.tsv')
        ...
        for i in range(args.num_epochs):
            ...
            for j in range(STEPS):
                ...
                if end > start:
                    ...
                    x1_tp1 = np.concatenate((x1_tp[start:], x1_tp[:end]), axis=0)
                ...
                sess.run(birnnatt.train_op, feed_dict={birnnatt.input_citation: x1_citation1,
                                                       ...
                                                       birnnatt.input_tp: x1_tp1,
                                                       birnnatt.input_l1: y1_l11,
                                                       birnnatt.input_l2: y1_l21})

                if j % 200 == 0:
                    model_loss, model_loss1 = sess.run((birnnatt.loss, birnnatt.loss1), feed_dict={birnnatt.input_citation: x2_citation,
                                                                                                   birnnatt.input_title: x2_title,
                                                                                                   birnnatt.input_content: x2_content,
                                                                                                   birnnatt.input_tp: x2_tp,
                    ...
                    if min_cross > model_loss1:
                        ...
                        output1 = open((args.result_path + 'net6-result'+args.para+'.txt'), 'w')
                        output2 = open((args.result_path + 'net6-az-result'+args.para+'.txt'), 'w')
