...
def load_data_and_labels(filepath):
    ...
    with open(filepath, 'r', encoding="utf-8") as f:
        for line in f:
            ...
            content.append(parts[3])
            if parts[4].startswith('0'):
                one_hot_labels.append([0, 1])
            else:
                one_hot_labels.append([1, 0])
            if parts[5].startswith('1'):
                az_labels.append([1, 0, 0, 0])
            elif parts[5].startswith('2'):
                az_labels.append([0, 1, 0, 0])
            elif parts[5].startswith('4'):
                az_labels.append([0, 0, 1, 0])
            else:
                az_labels.append([0, 0, 0, 1])
    return [citation, np.array(tp_label), title, content, np.array(one_hot_labels), np.array(az_labels)]
