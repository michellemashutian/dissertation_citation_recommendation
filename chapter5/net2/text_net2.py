...
class TextNet2(object):
    ...
    def __init__(
            self, w2v_model, sequence_length1, sequence_length2, sequence_length3, layer1_dim, layer2_dim, layer3_dim,
            num_classes, az_classes, vocab_size,
        ...
        graph
        '''
        # self.y_pred, self.y_az = self.CNN
    ...
    def CNN(self):
        ...
        with tf.name_scope("score"):
            ...
            w_az = tf.get_variable("w_az", shape=[num_filters_total, self.az_classes], initializer=tf.contrib.layers.xavier_initializer())
            b_az = tf.Variable(tf.constant(0.1, shape=[self.az_classes]), name="b_az")
            az_scores = tf.nn.relu(tf.matmul(final_output1, w_az) + b_az)

        return scores, az_scores
